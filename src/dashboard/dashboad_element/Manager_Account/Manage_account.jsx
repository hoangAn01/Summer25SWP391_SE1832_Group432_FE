...
import { Table, Button, Space, Modal, message, Input, Popconfirm, Select } from 'antd';
...
function Manage_account() {
    ...
    if (!searchTerm) {
      ...
      const filtered = accounts.filter(account => {
        ...
        return (
          ...
          normalizedFullName.split(' ').some(namePart => 
            namePart.startsWith(normalizedSearchTerm) // Match start of any name part
          )
        );
      });
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(account => {
        ...
        return (
          ...
          normalizedFullName.split(' ').some(namePart => 
            namePart.startsWith(normalizedSearchTerm)
          )
        );
      });
    }
  ...
  const handleDelete = async (record) => {
    ...
    try {
      ...
      const updatedAccounts = accounts.filter(account => account.userID !== record.userID);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
      <div className="w-full max-w-2xl p-8 bg-white rounded-lg shadow-md mt-12">
        <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">Quản Lý Tài Khoản</h1>
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", margin: "40px 0" }}>
          <div style={{ position: "relative", width: 600, maxWidth: "98vw", display: 'flex', gap: '10px' }}>
            <Input
